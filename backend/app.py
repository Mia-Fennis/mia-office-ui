...
OPENCLAW_WORKSPACE = os.environ.get("OPENCLAW_WORKSPACE") or os.path.join(os.path.expanduser("~"), ".kimi_openclaw", "workspace")
...
ASSET_DRAWER_PASS_DEFAULT = os.getenv("ASSET_DRAWER_PASS", "")
# 若未设置环境变量，尝试从 runtime-config 读取；若仍为空，则在首次 /assets/auth 请求时生成
...
if is_production_mode():
    ...
    if not is_strong_drawer_pass(ASSET_DRAWER_PASS_DEFAULT):
        hardening_errors.append("ASSET_DRAWER_PASS is weak (need >=8 chars, non-default)")
    if hardening_errors:
...
def assets_auth_status():
    return jsonify({
        ...
        "drawer_default_pass": not is_strong_drawer_pass(ASSET_DRAWER_PASS_DEFAULT),
    })
